...
class traveldata extends Component
{
    ...
    componentDidMount(){
        var properties;
        var data={
            propertyid:this.props.ownerpropid
        }
        console.log("owner booking details"+data);
        axios.post('http://localhost:3001/fetchownerbookingdetails',data)
        .then((response) => {
        this.setState({
            trips : this.state.trips.concat(response.data.updatedList.result) 
        });
    });
    }
    render()
