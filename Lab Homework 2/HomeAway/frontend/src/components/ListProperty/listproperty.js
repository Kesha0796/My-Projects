...
class ListProp extends Component
{
    ...
    submitLogin = (e) => {
        ...
        this.props.onLocationHandle(data);
...
const mapDispatchStateToProps = dispatch => {
    return {
        onLocationHandle : (data) => {
            axios.post('http://localhost:3001/locdata', data)
                .then((response) => {
                    console.log("status "+response.status);
                    dispatch({type: 'LOCATIONPROP',payload : response.data.resData,statusCode : response.status})
            });
        }
    }
}
//export Login Component
export default connect(mapStateToProps,mapDispatchStateToProps)(ListProp);