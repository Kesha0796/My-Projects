...
require "db1.php";
$order2="Update complaint_details set Status=1 where 	ComplaintId='$did'";
	 $result2 = $conn->query($order2);
$order = "SELECT AadharNumber FROM complaint_details where ComplaintId=$did ";
        $result = $conn->query($order);
       // while($row = $result->fetch_assoc())
        //{
        $row = mysql_fetch_array($result);
	        $order1="SELECT Emailid FROM aadhar_details where AadharNumber=$row['AadharNumber']";
