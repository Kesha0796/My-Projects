...
class ListPropDetails extends Component
{
    ...
    submitLogin = (e) => {
        ...
        this.props.onDetailsHandle(data);
...
const mapDispatchStateToProps = dispatch => {
    return {
        onDetailsHandle : (data) => {
            axios.post('http://localhost:3001/detailsdata', data)
                .then((response) => {
                    dispatch({type: 'DETAILS',payload : response.data.resData,statusCode : response.status})
            });
        }
    }
}
//export Login Component
export default connect(mapStateToProps,mapDispatchStateToProps)(ListPropDetails);