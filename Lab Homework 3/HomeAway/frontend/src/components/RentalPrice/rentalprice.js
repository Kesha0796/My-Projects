...
const mapDispatchStateToProps = dispatch => {
    return {
        onPriceHandle : (data) => {
            axios.post('http://localhost:3001/pricedata', data)
                .then((response) => {
                    dispatch({type: 'PRICEDATA',payload : response.data.resData,statusCode : response.status})
            });
        }
    }
}
...
export default connect(mapStateToProps,mapDispatchStateToProps)(rentalprice);